...
$jobs += Start-Job -Name "InstallLibsApplication" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../application")
    abp install-libs

    if ($LASTEXITCODE -ne 0) {
        throw "abp install-libs (application) exited with code $LASTEXITCODE"
    }
}

$jobs += Start-Job -Name "InstallLibsBookManagement" -ScriptBlock {
    $ErrorActionPreference = "Stop"
    Set-Location (Join-Path $using:scriptRoot "../../modules/book-management")
    abp install-libs

    if ($LASTEXITCODE -ne 0) {
        throw "abp install-libs (book-management) exited with code $LASTEXITCODE"
    }
}
